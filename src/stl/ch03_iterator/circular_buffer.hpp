...
#include <stdexcept>
/** 
 * @brief CircularBuffer 中新增 iterator 类，使其支持迭代器操作
 */
...
class CircularBuffer {
   public:
    using value_type = T;
    CircularBuffer()
        : _head(0), _tail(0), _size(0), _current(0), _capacity(cap) {}
    ...
    std::size_t capacity() noexcept { return _capacity; }

    bool empty() const noexcept { return (int)size() <= 0; }
    ...
    ********************************************/
    friend std::ostream& operator<<(std::ostream& os,
        ...
        return (os << "head: " << buf._head << ",tail: " << buf._tail
                   << ",current: " << buf._current << ",capaciyt: "
                   << buf._capacity << ",size: " << buf.size());
    }

    /*
     * create a inner class to implement iterator
     */
    ...
    class CircularBufferIterator {
       public:
        ...
        pointer operator->() const { return _cursor; }
